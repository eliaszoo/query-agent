...
class TestSQLRiskChecker:
    ...
    def test_update_indexes(self):
        self.checker.update_indexes("default", "test", "tb_scene", [
            IndexInfo(table="tb_scene", name="PRIMARY", columns=["id"], unique=True, index_type="BTREE"),
        ])
        assert self.checker.has_indexes("default", "test", "tb_scene")

    def test_no_indexes(self):
        assert not self.checker.has_indexes("default", "test", "tb_scene")

    # --- 无 WHERE 条件 ---

    def test_no_where_no_limit_high_risk(self):
        """无 WHERE 且无 LIMIT → high risk。"""
        result = self.checker.check("default", "test", "SELECT * FROM tb_scene")
        assert result.has_risk
        assert result.risk_level == "high"

    def test_no_where_with_limit_still_risk(self):
        """无 WHERE 但有 LIMIT → 仍有 SELECT * 风险。"""
        result = self.checker.check("default", "test", "SELECT * FROM tb_scene LIMIT 10")
        assert result.has_risk
    ...
    def test_where_hits_index_prefix_no_risk(self):
        ...
        result = self.checker.check("default", "test", "SELECT id FROM tb_scene WHERE model_id = 1")
    ...
    def test_where_hits_composite_index_prefix_no_risk(self):
        ...
        result = self.checker.check("default", "test", "SELECT id FROM tb_scene WHERE status = 1")
        assert not result.has_risk
    ...
    def test_where_non_prefix_column_medium_risk(self):
        ...
        result = self.checker.check("default", "test", "SELECT id FROM tb_scene WHERE created_at > '2024-01-01'")
        assert result.has_risk
    ...
    def test_where_column_not_in_any_index(self):
        ...
        result = self.checker.check("default", "test", "SELECT id FROM tb_scene WHERE name = 'test'")
        assert result.has_risk
        assert result.risk_level == "high"
    ...
    def test_mixed_prefix_and_no_index(self):
        ...
        result = self.checker.check("default", "test",
    ...
    def test_select_star_medium_risk(self):
        ...
        result = self.checker.check("default", "test", "SELECT * FROM tb_scene WHERE id = 1")
        assert result.has_risk
    ...
    def test_leading_wildcard_like(self):
        ...
        result = self.checker.check("default", "test", "SELECT id FROM tb_scene WHERE name LIKE '%test%'")
        assert result.has_risk
    ...
    def test_no_index_info_no_where_high_risk(self):
        ...
        result = self.checker.check("default", "test", "SELECT id FROM tb_scene")
        assert result.has_risk
        assert result.risk_level == "high"
    ...
    def test_multi_table_join(self):
        ...
        self.checker.update_indexes("default", "test", "tb_model", [
    ...
    def test_driving_column_lowers_non_index_to_medium(self):
        ...
        self.checker.update_indexes("default", "test", "tb_scene_app", [
