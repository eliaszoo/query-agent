...
class FeishuBotService:
    ...
    def _cmd_help() -> str:
        lines = [
            ...
            "  解除业务锁定",
            "",
            "**/add <name> <sse_url> [key]**",
            "  添加 MCP Server（key 为可选的 Bearer Token）",
            "",
            "**/remove <name>**",
            "  移除 MCP Server",
            "",
            ...
            "",
            "**/clear [business]**",
            "  清除错误记忆，指定 business 则只清除该业务的",
            "",
            "**/fields**",
            "  列出所有字段知识",
            "",
            "**/field <table>.<column> <description>**",
            "  添加字段知识，如 /field tb_voice.status 1=正常,2=禁用",
            "",
            "**/field_rm <table>.<column>**",
            "  删除字段知识",
            "",
            "**/remember <rule>**",
            "  保存默认查询规则到当前业务，如 /remember 默认只查可用数据",
            "",
            "**/rules**",
            "  列出所有业务的默认查询规则",
            "",
            "**/rules_clear [business]**",
            "  清除默认查询规则，指定 business 则只清除该业务的",
            "",
            "**/prompt**",
            "  查看当前 system prompt",
            "",
            "**/plan <query>**",
            "  预览查询计划（不实际执行），如 /plan 查询数字人数量",
        ]
