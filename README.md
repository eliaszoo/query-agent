...
│                   MCP Server (服务端)               │
│                                                     │
│  Tools: execute_readonly_sql | get_table_schema     │
│         get_cluster_list    | get_table_indexes     │
│         get_business_knowledge                      │
│                       │                             │
│              SQL Validator + Connection Pool         │
│                       │                             │
│                  MySQL (只读)                        │
└─────────────────────────────────────────────────────┘
```
...
```
src/
├── main.py              # CLI 入口，REPL 循环，反馈检测
├── agent.py             # QueryAgent 核心：对话循环、工具路由、风险检测
├── llm_provider.py      # LLM 抽象层（Anthropic / OpenAI Compatible）
├── business_registry.py # 多业务 MCP 连接管理（SSE 懒连接）
├── prompts.py           # System Prompt 构建（单业务/多业务）
...
└── query_mcp_server.py  # MCP Server（FastMCP + Bearer Auth）
```
